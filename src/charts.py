...
def yearly_population_bar_chart(df, csv_row, year_cols, county_name):
    ...
    chart = (
        ...
        .properties(
            width=400,
            height=350,
            title=f"{county_name}: Population by Year"
        )
    )
...
def top_growth_leaderboard_chart(
    ...
    }).sort_values("GrowthPct", ascending=False).head(top_k)
...
def forecast_population_chart(county_name, df, year_cols):
    ...
    chart = (
        ...
        .properties(
            width=380,
            height=300,
            title=f"Population Forecast for {county_name}"
        )
    )
...
def growth_correlation_heatmap(df: pd.DataFrame, year_cols):
    ...
    chart = (
        ...
        .properties(
            width=380,
            height=380,
            title="Correlation of Year-over-Year Growth"
        )
    )
